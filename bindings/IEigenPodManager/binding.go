...
func (_ContractIEigenPodManager *ContractIEigenPodManagerCaller) MaxPods(opts *bind.CallOpts) (*big.Int, error) {
	var out []interface{}
	err := _ContractIEigenPodManager.contract.Call(opts, &out, "maxPods")
...
//
// Solidity: function maxPods() view returns(uint256)
func (_ContractIEigenPodManager *ContractIEigenPodManagerSession) MaxPods() (*big.Int, error) {
	return _ContractIEigenPodManager.Contract.MaxPods(&_ContractIEigenPodManager.CallOpts)
}

// MaxPods is a free data retrieval call binding the contract method 0xc0ccbf10.
//
// Solidity: function maxPods() view returns(uint256)
func (_ContractIEigenPodManager *ContractIEigenPodManagerCallerSession) MaxPods() (*big.Int, error) {
	return _ContractIEigenPodManager.Contract.MaxPods(&_ContractIEigenPodManager.CallOpts)
}
...
// ContractIEigenPodManagerMaxPodsUpdatedIterator is returned from FilterMaxPodsUpdated and is used to iterate over the raw logs and unpacked data for MaxPodsUpdated events raised by the ContractIEigenPodManager contract.
type ContractIEigenPodManagerMaxPodsUpdatedIterator struct {
	Event *ContractIEigenPodManagerMaxPodsUpdated // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *ContractIEigenPodManagerMaxPodsUpdatedIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(ContractIEigenPodManagerMaxPodsUpdated)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *ContractIEigenPodManagerMaxPodsUpdatedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *ContractIEigenPodManagerMaxPodsUpdatedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// ContractIEigenPodManagerMaxPodsUpdated represents a MaxPodsUpdated event raised by the ContractIEigenPodManager contract.
type ContractIEigenPodManagerMaxPodsUpdated struct {
...
func (_ContractIEigenPodManager *ContractIEigenPodManagerFilterer) FilterMaxPodsUpdated(opts *bind.FilterOpts) (*ContractIEigenPodManagerMaxPodsUpdatedIterator, error) {

	logs, sub, err := _ContractIEigenPodManager.contract.FilterLogs(opts, "MaxPodsUpdated")
	if err != nil {
		return nil, err
	}
	return &ContractIEigenPodManagerMaxPodsUpdatedIterator{contract: _ContractIEigenPodManager.contract, event: "MaxPodsUpdated", logs: logs, sub: sub}, nil
}
...
//
// Solidity: event MaxPodsUpdated(uint256 previousValue, uint256 newValue)
func (_ContractIEigenPodManager *ContractIEigenPodManagerFilterer) WatchMaxPodsUpdated(opts *bind.WatchOpts, sink chan<- *ContractIEigenPodManagerMaxPodsUpdated) (event.Subscription, error) {

	logs, sub, err := _ContractIEigenPodManager.contract.WatchLogs(opts, "MaxPodsUpdated")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(ContractIEigenPodManagerMaxPodsUpdated)
				if err := _ContractIEigenPodManager.contract.UnpackLog(event, "MaxPodsUpdated", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event MaxPodsUpdated(uint256 previousValue, uint256 newValue)
func (_ContractIEigenPodManager *ContractIEigenPodManagerFilterer) ParseMaxPodsUpdated(log types.Log) (*ContractIEigenPodManagerMaxPodsUpdated, error) {
	event := new(ContractIEigenPodManagerMaxPodsUpdated)
	if err := _ContractIEigenPodManager.contract.UnpackLog(event, "MaxPodsUpdated", log); err != nil {
		return nil, err
	}
