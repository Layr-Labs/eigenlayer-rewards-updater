...
// FetchPostedRewards provides a mock function with given fields: ctx
func (_m *ProofDataFetcher) FetchPostedRewards(ctx context.Context) ([]*proofDataFetcher.SubmittedRewardRoot, error) {
	ret := _m.Called(ctx)

	if len(ret) == 0 {
		panic("no return value specified for FetchPostedRewards")
	}

	var r0 []*proofDataFetcher.SubmittedRewardRoot
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context) ([]*proofDataFetcher.SubmittedRewardRoot, error)); ok {
		return rf(ctx)
	}
	if rf, ok := ret.Get(0).(func(context.Context) []*proofDataFetcher.SubmittedRewardRoot); ok {
		r0 = rf(ctx)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]*proofDataFetcher.SubmittedRewardRoot)
		}
	}
